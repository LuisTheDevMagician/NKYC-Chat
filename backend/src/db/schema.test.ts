...
describe("createDb", () => {
  it("applies migrations creating users, sessions, conversations, and messages tables", () => {
    ...
    expect(tables.map((t) => t.name)).toEqual(["conversations", "messages", "sessions", "users"]);
  });
});
